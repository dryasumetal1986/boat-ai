...
if st.button("🚀 AI予想を実行", type="primary"):
    ...
    # --- オッズデータ取得 & 妙味（期待値）解析 ---
    odds_trifecta = race.get("odds", {}).get("trifecta", {}) # 3連単オッズ
    odds_exacta = race.get("odds", {}).get("exacta", {})     # 2連単オッズ
        ...
        for j in lanes:
            for k in lanes:
                if i != j and j != k and i != k:
                    ...
                    odds_val = num(odds_trifecta.get(f"{i}{j}{k}", 0))
                    expected_value = round(prob * odds_val, 2)
                    predictions_3t.append({
                        "組み合わせ": combo,
    ...
    pred_3t_df = pd.DataFrame(predictions_3t)
    ...
    for i in lanes:
        for j in lanes:
            if i != j:
                combo = f"{i}-{j}"
                prob = (score_map[i] / total_score) * (score_map[j] / (total_score - score_map[i] + 0.1))
                odds_val = num(odds_exacta.get(f"{i}{j}", 0))
                expected_value = round(prob * odds_val, 2)
                predictions_2t.append({
                    "組み合わせ": combo,
                    "AI理論勝率": f"{round(prob * 100, 1)}%",
                    "オッズ": f"{odds_val}倍" if odds_val > 0 else "未確定",
                    "期待値スコア": expected_value if odds_val > 0 else prob * 10,
    ...
    pred_2t_df = pd.DataFrame(predictions_2t)

    st.subheader("🎯 AI推薦買い目")
    tab_3t, tab_2t = st.tabs(["3連単 予想", "2連単 予想"])
    ...
    with tab_3t:
        ...
        with col_h3:
            st.write("### 🟢 本命狙い（勝率重視）")
            top_prob = pred_3t_df.sort_values("AI理論勝率", ascending=False).head(5)
            for _, row in top_prob.iterrows():
                st.success(f"**{row['組み合わせ']}** | 勝率: {row['AI理論勝率']} | オッズ: {row['オッズ']}")
        with col_m3:
            st.write("### 🟡 妙味狙い（回収率・高期待値）")
            if (pred_3t_df["raw_odds"] > 0).any():
                top_exp = pred_3t_df.sort_values("期待値スコア", ascending=False).head(5)
                for _, row in top_exp.iterrows():
                    st.warning(f"**{row['組み合わせ']}** | 期待値: {row['期待値スコア']} | オッズ: {row['オッズ']}")
            else:
                st.info("※現在オッズが未確定のため、評価スコアベースで表示中")
                top_exp = pred_3t_df.sort_values("期待値スコア", ascending=False).head(5)
                for _, row in top_exp.iterrows():
    ...
    with tab_2t:
        ...
        with col_h2:
            st.write("### 🟢 本命狙い（勝率重視）")
            top_prob_2t = pred_2t_df.sort_values("AI理論勝率", ascending=False).head(4)
            for _, row in top_prob_2t.iterrows():
                st.success(f"**{row['組み合わせ']}** | 勝率: {row['AI理論勝率']} | オッズ: {row['オッズ']}")
        with col_m2:
            st.write("### 🟡 妙味狙い（回収率・高期待値）")
            if (pred_2t_df["raw_odds"] > 0).any():
                top_exp_2t = pred_2t_df.sort_values("期待値スコア", ascending=False).head(4)
                for _, row in top_exp_2t.iterrows():
                    st.warning(f"**{row['組み合わせ']}** | 期待値: {row['期待値スコア']} | オッズ: {row['オッズ']}")
            else:
                st.info("※現在オッズが未確定のため、評価スコアベースで表示中")
                top_exp_2t = pred_2t_df.sort_values("期待値スコア", ascending=False).head(4)
