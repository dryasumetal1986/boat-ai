...
if pred:
    ...
    st.write(
        f"📍 {data.get_stadium_name(stadium)} {race_no}R"
    )
    ...
    for item in candidates:
        ...
        market = item["market_probability"]
        edge = item["edge"]

        st.write(
            f"🎯 {combo[0]}-{combo[1]}-{combo[2]} "
            f"オッズ：{odd:g}倍 "
            f"AI確率：{probability * 100:.3f}% "
            f"市場確率：{market * 100:.3f}% "
            f"EV：{ev_rate:+.1f}% "
            f"Edge：{edge * 100:+.3f}%"
        )
...
if bt:
    ...
    for item in results[start:end]:
        ...
        actual3 = tuple(actual[:3])

        hit = (
            len(actual3) == 3
            and predicted == actual3
        )
